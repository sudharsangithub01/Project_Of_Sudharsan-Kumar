...
SELECT brd.bidder_id,sum(CASE WHEN bid_status = 'WON' THEN 1 ELSE 0 END) as Won,
 SUM(CASE WHEN bid_status = 'lost' THEN 1 ELSE 0 END) as Lost , no_of_bids, 
 round(sum(CASE WHEN bid_status = 'WON' THEN 1 ELSE 0 END)/NO_OF_BIDS*100,2) as Percent_Won, 
 round(sum(CASE WHEN bid_status = 'Lost' THEN 1 ELSE 0 END)/NO_OF_BIDS*100,2) as Percent_Lost
 FROM ipl_bidding_details brd JOIN ipl_bidder_points brp 
 using(bidder_id)
 GROUP BY bidder_id 
 ORDER BY percent_won DESC;

2.	Display the number of matches conducted at each stadium with stadium name, city from the database.

SELECT count(ims.match_id) match_id,ims.STADIUM_ID ,ips.STADIUM_NAME ,ips.CITY
...
SELECT std.stadium_id, std.stadium_name,
 round((sum(case when toss_winner = match_winner then 1 else 0 end)/count(match_id))*100,2) as Won_Both_Toss_and_match
 FROM ipl_match m JOIN ipl_match_schedule 
 using (match_id) JOIN ipl_stadium std using (stadium_id)
GROUP BY std.stadium_id;
...
SELECT team_id1 AS team_id, win_details ,match_winner
FROM ipl_match 
WHERE win_details LIKE "%WON%"
AND MATCH_WINNER=1
UNION ALL
SELECT team_id1 AS team_id, win_details ,match_winner
FROM ipl_match 
...
SELECT it.team_id,player_name,team_name,player_role
FROM  ipl_player ip
JOIN ipl_team_players itp
ON ip.player_id= itp.player_id
JOIN ipl_team it
ON it.team_id=itp.team_id
WHERE PLAYER_ROLE LIKE "%BOWLER%"
AND TEAM_NAME LIKE "Mumbai Indians"

8.	How many all-rounders are there in each team, Display the teams with more than 4 
all-rounder in descending order.

SELECT tp.team_id, count(tp.player_id) count_players, tp.player_role, t.team_name
 FROM ipl_team_players tp JOIN ipl_team t using (team_id) 
 WHERE tp.player_role like 'All-Rounder'
 GROUP BY tp.team_id having count_players > 4 ORDER BY count_players desc
